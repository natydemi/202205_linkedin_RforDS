...
E para discutir melhor estes grupos, vamos considerar 
```{r}
youtube %>% select(where(is.numeric)) %>% slice(20:36)
```
...
```{r}
youtube %>% slice(20:31) %>% pull(dislike_count) %>% sort()
```


```{r}
youtube %>% slice(20:30) %>% select(dislike_count) %>% skimr::skim()
```
