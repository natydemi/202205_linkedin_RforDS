...
> As funções `sample_frac()` e `sample_n()` estão sendo substituidas pela função `slice_sample()`, no caso, com os parâmetros `slice_sample(frac = *)` e `slice_sample(n = *)`, respectivamente.
...
```{r}
voos %>% 
  summarise(mean_distancia = mean(distancia),
            mediana_distancia = quantile(distancia, 0.5)) %>% 
...
voos %>%
  group_by(mes) %>% 
  summarise(tempo_voo_mean = mean(tempo_voo, na.rm = T))  %>% 
  arrange(-tempo_voo_mean)
``` 
...
voos_atrasados %>%
  ...
  summarise(n = n(),
            mean_distancia = mean(distancia)) # o número de linhas em cada grupo
``` 
