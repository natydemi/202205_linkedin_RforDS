...
Uma parte fundamental da análise é trabalhar com dados confiáveis e que estejam em boa qualidade. Sabemos que essa nem sempre (quase nunca!) é a realidade, por isso precisamos aprender a arrumar os dados e ter certeza que não há nenhuma inconsistência antes de fazer qualquer inferência.

Além disso, muitas vezes é necessário transformar uma base de dados inicial, derivando diferentes tabelas que ajudrão em análises mais aprofundadas, ou ainda gráficos e visualizações para que você ou sua audiência entendam as informações contidas ali.
...
Na descrição do pacote, o `dplyr` é definido como a gramática para transformação de dados, e esse nome não é à toa. Ele possui vários verbos que definem ações específicas para lidar com bases de dados, visando modificá-las de forma simples e eficiente.
...
Para isso, ele usa as colunas especificadas na coluna parâmetro `key`, e seus valores na coluna parâmetro `value`. Assim conseguimos tratar os dados como nos exemplos acima, seja filtrando, escrevendo novas variáveis, transformando as já existentes, e mais.
...
Como ferramenta transversal a todas as etapas do workflow de análise, temos a programação funcional, que permite a criação de códigos mais rápidos, simples e reprodutíveis.
